PSO_DLS = function(config){
	...
	calculateBest = function(particles, lbest=NULL) {
		...
		if(!is.null(lbest)){
			for(s in 1:config$sub_swarms){
				if(localBest[[s]]$val == lbest[[s]]$val){			
					for(i in 1:length(particles)){
						swarm = particles[[i]]$swarm
						if(swarm == s && particles[[i]]$best$val != lbest[[s]]$val){
							
							particles[[i]]$best$val < localBest[[swarm]]$val
							localBest[[swarm]] = particles[[i]]$best
						}
						if(particles[[i]]$best$val < globalBest$val){
							globalBest = particles[[i]]$best
	...
	for(it in 1:config$iterations){		
		...
		best = calculateBest(particles, lbest);	
		gbest = best[[1]]
		lbest = best[[2]]
		unitedLbest = best[[3]]
